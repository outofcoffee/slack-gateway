...
class SlackOutboundMessageService @Inject constructor(
    private val slackOperationsService: io.gatehill.slackgateway.backend.slack.service.SlackOperationsService
) : OutboundMessageService {

    private val logger: Logger =
        LogManager.getLogger(io.gatehill.slackgateway.backend.slack.service.SlackOutboundMessageService::class.java)
    ...
    private val channelCache = CacheBuilder.newBuilder()
        ...
        .build<ChannelType, List<io.gatehill.slackgateway.backend.slack.model.SlackChannel>>()
    ...
    private fun searchCacheForChannel(
        channelName: String,
        channelType: ChannelType
    ): io.gatehill.slackgateway.backend.slack.model.SlackChannel? =
        channelCache.getIfPresent(channelType)?.firstOrNull { it.name == channelName }

    private fun searchSlackApiForChannel(
        channelName: String,
        channelType: ChannelType
    ): io.gatehill.slackgateway.backend.slack.model.SlackChannel? {
    ...
    private fun checkParticipants(channel: io.gatehill.slackgateway.backend.slack.model.SlackChannel) {
        if (io.gatehill.slackgateway.backend.slack.config.SlackSettings.inviteGroups.isEmpty() && io.gatehill.slackgateway.backend.slack.config.SlackSettings.inviteMembers.isEmpty()) {
        ...
        io.gatehill.slackgateway.backend.slack.config.SlackSettings.inviteGroups.forEach { userGroupName ->
            try {
        ...
        memberIds += io.gatehill.slackgateway.backend.slack.config.SlackSettings.inviteMembers.mapNotNull { memberUsername ->
        ...
        memberIds
            ...
            .forEach { memberId ->
                ...
                try {
                    ...
                    logger.warn(
                        "Error inviting member $memberId to channel ${channel.name} (${channel.channelType}) - continuing",
                        e
                    )
                }
            }
